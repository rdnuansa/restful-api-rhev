...
if (empty($arr_input)) {
   ...
   echo json_encode(array("response" => "400", "message" => "[Unable to start VM. Data is empty.]"), JSON_PRETTY_PRINT);
	...
	if(!$vm->isOvercommit()) {
		...
		// Send response overcommit already reached threshold limit value
     	http_response_code(406);
     	echo json_encode(array("response" => "406", "message" => "[Overcommit ratio exceed 100%! Please power off another VM first.]"), JSON_PRETTY_PRINT);
